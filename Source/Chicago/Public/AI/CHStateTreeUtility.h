...
struct FStateTreeAdjustCoverPositionInstanceData
{
	...
	UPROPERTY(EditAnywhere, Category = Context)
	FVector RoughPosition;

	UPROPERTY(EditAnywhere, Category = Output)
	FVector AdjustedCoverPosition;
};


USTRUCT(meta=(DisplayName = "Adjust Cover Position", Category="Chicago"))
struct FStateTreeAdjustCoverPositionTask : public FStateTreeTaskCommonBase
{
	GENERATED_BODY()

	using FInstanceDataType = FStateTreeAdjustCoverPositionInstanceData;
	virtual const UStruct* GetInstanceDataType() const override {return FInstanceDataType::StaticStruct();}
